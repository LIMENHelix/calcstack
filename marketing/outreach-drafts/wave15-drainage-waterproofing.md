# Wave 15 outreach drafts — drainage & waterproofing (French drain, dry well, sump pump)

The hook: wet-basement and yard-drainage content is evergreen, and every article in the niche
teaches the same material list — washed #57 stone, holes-down pipe, fabric wrap, 1% slope. Our
French drain calculator quantifies that list with pipe displacement subtracted and a live slope
check, and it completes a water-management chain no competitor has: gutters → roof drainage →
French drain → dry well → sump pump → the excavation to dig it all. Send after Wave 14.
Personalize every [bracket]. One per day.

Embed gallery: https://calcstack-eight.vercel.app/embeds

---

## 1. Basement waterproofing blogs (Basement Systems dealer blogs, waterproofing trade sites) — Template A

Subject: A French drain calculator with the slope check built in

> Hi [name],
>
> Every French drain article ends with the same rules — washed #57 stone, holes DOWN, fabric
> burrito wrap, 1% minimum slope — and every reader still guesses the gravel tonnage. Our
> calculator quantifies the whole bill of materials: trench volume minus pipe displacement for
> true gravel tons, perforated pipe footage with fitting allowance, fabric square footage for
> the wrap, and a live slope check against the fall available to the outlet:
>
> Live demo: https://calcstack-eight.vercel.app/embed/french-drain-calculator
> Full page: https://calcstack-eight.vercel.app/calculators/french-drain-calculator
>
> The 50-ft worked example on the page reconciles to 2.6 yards / 3.7 tons — the same number
> your estimators would quote. And where there's no daylight outlet, the chain continues:
> a dry well calculator (roof runoff ÷ the 40% gravel void ratio → pit count and stone
> tonnage, the NJ DEP sizing method) and a sump pump sizer (the 60-second pit rise test →
> GPH at your total dynamic head → the right horsepower, not the box number):
>
> https://calcstack-eight.vercel.app/embed/dry-well-calculator
> https://calcstack-eight.vercel.app/embed/sump-pump-calculator
>
> One iframe embeds any of them on a drainage article. Free forever, no signup, all math
> in the browser.
>
> [Name], CalcStack

## 2. Landscape supply yards & gravel retailers — Template B (commercial angle)

Subject: Send your customers a calculator that orders your stone correctly

> Hi [name],
>
> The most common counter conversation at a landscape yard: "how much stone for a French
> drain?" Our free calculator answers it with your exact product — washed #57 by the ton —
> including the pipe displacement most DIYers forget, so they order once and order right:
>
> https://calcstack-eight.vercel.app/embed/french-drain-calculator
>
> Yards that embed a calculator like this see fewer under-orders (the second delivery fee is
> where customer goodwill dies) and fewer returns. It also computes gravel for driveways and
> road base with the compaction factor built in:
> https://calcstack-eight.vercel.app/embed/road-base-calculator
>
> One iframe on your stone product pages. Free forever, no signup.
>
> [Name], CalcStack

## 3. Gutter & roofing companies with content marketing — Template A

Subject: The water chain after the downspout — embeddable drainage calculators

> Hi [name],
>
> Your content already tells homeowners that downspouts dumping at the foundation are half
> the wet-basement problem. The fix is a drain line — and we built the calculator for it:
> trench size to gravel tons, pipe footage, fabric, and the 1% slope check:
>
> https://calcstack-eight.vercel.app/embed/french-drain-calculator
>
> It pairs naturally with our gutter sizing calculator (drainage area × rainfall intensity →
> gutter and downspout sizes), so your readers can go from "is my gutter big enough" to
> "where does the water go" on one page:
> https://calcstack-eight.vercel.app/embed/gutter-size-calculator
>
> Both embed with one iframe, free forever, no signup.
>
> [Name], CalcStack

## 4. Lawn care & landscaping YouTubers / creator resource pages — Template A

Subject: Drainage job math for your viewers — embeddable and free

> Hi [name],
>
> Drainage videos are some of the highest-watch-time content in the lawn niche, and the
> pinned comment is always "how much gravel?" Our French drain calculator is the answer
> card: gravel tons with pipe displacement out, pipe and fabric quantities, slope check,
> and a material cost line:
>
> https://calcstack-eight.vercel.app/calculators/french-drain-calculator
>
> It embeds on a website or links clean in a description — free forever, no signup, runs
> entirely in the viewer's browser. Happy to build any spec variation your audience asks for.
>
> [Name], CalcStack

## 5. Reddit r/HomeImprovement & r/landscaping drainage threads — community protocol, NO pitch

Wet-basement and soggy-yard threads are weekly fixtures. Strict norms:
- Diagnose first: where's the water coming from, where can it daylight to.
- Give the worked math inline (trench volume, slope check) before any link.
- Link only when the output IS the answer; 10+ real answers before the first one.
